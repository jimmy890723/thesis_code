...
import random
import re
import MBTI_prediction_by_BERT
import MBTI_prediction_by_3ways
...
async def MBTI_test(ctx):
    ...
    for i in range(total_question_num):
        ...
        poll_embed.set_author(name=ctx.author.display_name)
    ...
    # 等待投票結束
    while True:
        await asyncio.sleep(10)  # 每隔 10 秒檢查一次投票狀態
    ...
    if new_role:
        ...
        MBTI_record[member.id].clear()
        MBTI_record[member.id].append(MBTI_current)
    else:
...
async def on_message(message):
    global bot_temperature, characters, sentiments, response_record,member_stats, bot_mbti_TF, bot_round_count, MBTI_record, first_round_finish, mem_res_num, counter, check_user_roles, check_bot_roles
    ...
    if author_id not in member_stats:
        ...
        bot_round_count[author_id] = 0
        MBTI_record[author_id] = []
        first_round_finish[author_id] = False
        counter[author_id] = 0
        for role in aut_roles:
            if role.name.startswith("MBTI"):
                MBTI_now = role.name.split(': ',1)[1]
                MBTI_record[author_id].append(MBTI_now)
    
    check_user_roles[author_id] = False
    check_bot_roles[author_id] = False

    for role in aut_roles:
        if role.name.startswith("MBTI"):
            MBTI_now = role.name.split(': ',1)[1]
            check_user_roles[author_id] = True
        if role.name.startswith("BOT_MBTI"):
            BOT_MBTI_now = role.name.split(': ',1)[1]
            check_bot_roles[author_id] = True    
    
    if check_user_roles[author_id] == False:
        if message.content.startswith('$BOT_MBTI_') or message.content.startswith('$MBTI_records'):
            await message.channel.send("目前沒有" + message.author.name + "的MBTI人格身分紀錄。\n")
    else:
        if message.content.startswith('$BOT_MBTI_on'):
            bot_mbti_TF[author_id] = True
            if check_bot_roles[author_id] == False:
                BOT_MBTI_now = find_mbti_match(MBTI_now)
                for role in aut_roles:
                    if role.name.startswith("BOT_MBTI"):
                        old_role_name = role.name
                ...
                new_role_name = "BOT_MBTI : " + BOT_MBTI_now
                ...
                if new_role:
                    ...
                    print('找不到'+ new_role_name +'該身分組')
            await message.channel.send("已將bot套用 "+ BOT_MBTI_now +" 的人格進行回覆\n")
        elif message.content.startswith('$BOT_MBTI_off'):
            bot_mbti_TF[author_id] = False
            await message.channel.send("已停用bot的MBTI人格\n")
        elif message.content.startswith('$BOT_MBTI_change'):
            if check_bot_roles[author_id] == False:
                await message.channel.send("目前沒有" + message.author.name + "對應的bot的MBTI身分紀錄，請先輸入$BOT_MBTI_on後再使用此功能\n")
            else:
                BOT_MBTI_bef = BOT_MBTI_now
                BOT_MBTI_now = find_mbti_match(MBTI_now)
                for role in aut_roles:
                    if role.name.startswith("BOT_MBTI"):
                        old_role_name = role.name
                ...
                if new_role:
                    ...
                    print('找不到'+ new_role_name +'該身分組')
                await message.channel.send("已將bot從 "+ BOT_MBTI_bef +" 的人格切換到 " + BOT_MBTI_now +" 的人格\n")    
        else:
            print(message.author.name + "目前的對話人格是" + MBTI_now + "\n") 

    if message.content.startswith('$指令'):
        await message.channel.send("與bot對話 -\n@" + bot.user.name + " 要講的話 (採用Model:Davanci)\n@" + bot.user.name + " @@ 要講的話 (採用Model:Turbo)\n@" + bot.user.name + " $clear : 清除對話記錄 \n $測試流程 : 輸出研究測試流程\n $MBTI_16P: 16Personalities人格測試網站連結\n $MBTI_test : MBTI初始人格測試 \n $BOT_MBTI_on/off : bot是否套用MBTI人格\n $BOT_MBTI_change : 更改bot的MBTI人格\n $MBTI_records : 輸出使用者MBTI記錄\n $回饋表單 : 回饋表單連結\n")
    
    if message.content.startswith('$MBTI_records'):
        if not MBTI_record[author_id]:
            await message.channel.send("您目前沒有MBTI的相關紀錄")
        else:
            await message.channel.send("您的MBTI記錄為:" + str(MBTI_record[author_id]))

    if message.content.startswith('$MBTI_16P'):
        await message.channel.send("16Personalities網站連結:\n https://www.16personalities.com/tw\n 小提醒: 完成後請記下您在該網站的MBTI人格，並填寫於最後的回饋表單內，感謝您的配合!")

    if message.content.startswith('$測試流程'):
        await message.channel.send("1. 到16Personalities網站完成測試\n2. 輸入$MBTI_test完成測試\n3. @"+ bot.user.name +"@@ 要講的話 (8句)\n4. 輸入$BOT_MBTI_on \n5. 輸入@" + bot.user.name + "$clear \n6. 輸入@" + bot.user.name + "@@ 要講的話 (2個輪次,1個輪次8句話)\n7. 表單填寫成果\n\n指令懶人包:\n$MBTI_16P -> $MBTI_test -> @" + bot.user.name + "@@ 要講的話 (約8句話) -> $BOT_MBTI_on -> @" + bot.user.name + "$clear -> @@ 要講的話 (2個輪次,1個輪次8句話) -> 表單填寫成果")

    if message.content.startswith('$回饋表單'):
        await message.channel.send("回饋表單連結:\n https://forms.gle/QBNNodmsYd1FkAFh7")
    ...
    #檢查消息中是否提到了機器人
        ...
        try:
            ...
            if "$clear" in message.content:
                    ...
                    bot_round_count[author_id] = 0
                    first_round_finish[author_id] = False
                    await message.channel.send("messages history has been cleared")
                ...
                msg_final = message.content.split('@@',1)[1]
                #msg_setting = [{"role": "user", "content": "回覆的字數請不要超過對方回話的字數再加上20字。"}]
                msg_setting = [{"role": "system", "content": "回答問題的回覆字數不超過60個字，一般聊天或簡短對話則不超過30字。"}]
                #msg_setting.append({"role": "system", "content": "請在每次回覆後都加上一個表情符號"})
                if bot_mbti_TF[author_id] == True:
                    msg_setting.append({"role": "system", "content": "回覆的人格請採用" + BOT_MBTI_now})
                    msg_setting.append({"role": "system", "content": "你回覆的對象人格為" + MBTI_now})
                ...
                if len(response_record[author_id]) == mem_res_num:  #紀錄10句回覆的對話
                    for i in range(len(response_record[author_id])-2):
                        response_record[author_id][i] = response_record[author_id][i+2]
                    response_record[author_id][mem_res_num-2] = msg_final
                    response_record[author_id][mem_res_num-1] = response.choices[0].message.content
                else:    
    ...
    #處理對話記錄(記錄超過mem_res_num句)
    if not message.content.startswith('$'): 
        if member_stats[author_id] != 0 and member_stats[author_id]%mem_res_num == 0:
            if bot_mbti_TF[author_id] == False:
                await message.channel.send('\n\n提醒您，目前和機器人對話數量已達' + str(member_stats[author_id]) +'句，可切換型態($BOT_MBTI_on)，並清除前段記憶(@' + bot.user.name + '$clear)進行後續的測試。')
            else:
                counter[author_id] = 0
                msg_list = []
                pattern = r'<@(\w+)>'
                async for msg in channel.history(limit=1000):
                    if msg.author.id == author_id and '$' not in msg.content:
                        ...
                        if '@@' in msg.content:
                            ...
                            counter[author_id] += 1 #如果下面的註解刪了，這行要刪掉
                        '''
                        elif match:
                            print('check')
                            msg_list.append(msg.content.split('>',1)[1])
                        ...
                        counter[author_id] += 1
                        '''
                        if counter[author_id] == mem_res_num:
                            bot_round_count[author_id] += 1
                            await message.channel.send('\n已完成' + str(mem_res_num) +'句對話。正在為您的對話進行辨識，請稍後。\n')
                            break

                print("---分隔線---")
                print(msg_list) 
                member_stats[author_id] = 0
                MBTI_By_BERT, MBTI_list, MBTI_By_BERT_entire = MBTI_prediction_by_BERT.run(msg_list,MBTI_now)
                MBTI_By_SVM = MBTI_prediction_by_3ways.run(MBTI_list)
                print("BERT:" + MBTI_By_BERT_entire + " 3Ways:" + MBTI_By_SVM + " NOW:" + MBTI_now + "\n")
                new_string = ""
                # 确保三个字符串长度相同，如果不同可以根据需求进行适当处理
                if len(MBTI_By_BERT_entire) != len(MBTI_now) or len(MBTI_now) != len(MBTI_By_SVM):
                    print("MBTI字串長度不相同\n")
                else:
                    for i in range(len(MBTI_now)):
                        if MBTI_By_BERT_entire[i] == MBTI_now[i] or MBTI_By_SVM[i] == MBTI_now[i]:
                            new_string += MBTI_now[i]
                        else:
                            new_string += MBTI_By_BERT_entire[i]

                MBTI_now = new_string
                BOT_MBTI_now = find_mbti_match(MBTI_now)

                for role in aut_roles:
                    if role.name.startswith("MBTI") or role.name.startswith("BOT_MBTI"):
                        old_role_name = role.name
                        old_role = discord.utils.get(message.guild.roles, name=old_role_name)
                        if old_role:
                            await message.author.remove_roles(old_role)
                ...
                new_role_names = ["MBTI : " + MBTI_now , "BOT_MBTI : " + BOT_MBTI_now]
                check_MBTI_record = False
                for new_role_name in new_role_names:
                    new_role = discord.utils.get(message.guild.roles, name=new_role_name)
                    if new_role:
                        await message.author.add_roles(new_role)
                        print(f'已新增 {message.author.name} 的身分組 {new_role.name}')
                        if check_MBTI_record == False:
                            MBTI_record[message.author.id].append(MBTI_now)
                            check_MBTI_record = True    

    if bot_round_count[author_id] == 1 and first_round_finish[author_id] == False:
        await message.channel.send("\n\n提醒:您已完成1個輪次的機器人對話，再1個輪次即可結束，感謝您!\n")
        first_round_finish[author_id] = True
    elif bot_round_count[author_id] >= 2:
        await message.channel.send("\n\n提醒:您已完成2個輪次的機器人對話，可以點擊以下連結完成表單回饋，感謝您!\n https://forms.gle/cmuZ1eiFCfRjbX2y5")
        bot_round_count[author_id] = 0
        first_round_finish[author_id] = False
