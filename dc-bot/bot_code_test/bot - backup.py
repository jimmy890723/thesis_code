        ...
        try:
            ...
            if "$$" in message.content:
                ...
                response = openai.ChatCompletion.create(
                    model="gpt-3.5-turbo",
                    messages=[
                            {"role": "system", "content": "回覆的對話不能超過20個字"},  #只能下關於功能方面的設定 ex: 將中文轉換成英文 or 回覆的對話不超過20個字
                            {"role": "assistant", "content": f"{response_record[0] + response_record[1]}"},
                            {"role": "user", "content": f"{msg_final}"}
                        ]
                )
                await message.channel.send(response.choices[0].message.content)
                response_record.clear()
                response_record.append("user:" + msg_final)
                response_record.append("assistant: " + response.choices[0].message.content)
